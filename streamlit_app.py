...
# Set up rate limiter for Jina (20 requests per minute without API key)
...
def parse_jina_content(jina_data, original_url):
  if not jina_data or 'content' not in jina_data:
      return None
  ...
  return {
      ...
      'jina_content': content  # Store the full Jina content for further processing if needed
  }
...
async def extract_by_article(url, semaphore):
  async with semaphore:
      ...
      async with aiohttp.ClientSession(headers=make_header()) as session:
          async with session.get(jina_url, headers={"Accept": "application/json"}) as response:
              if response.status == 200:
                  jina_data = await response.json()
                  return parse_jina_content(jina_data, url)
              else:
                  print(f"Error fetching {url} from Jina: HTTP {response.status}")
                  return None
...
# The rest of your code (main_app, login_page, main) remains the same...
